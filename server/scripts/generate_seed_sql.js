...
async function generateSeed() {
    try {
        ...
        for (const row of users.rows) {
            stream.write(`INSERT INTO users (user_id, user_name, user_email, user_password, username, created_at, bio, profile_picture, linkedin_url, github_url, website_url, twitter_url, about, full_name, github_url, role) VALUES (${escapeSql(row.user_id)}, ${escapeSql(row.user_name)}, ${escapeSql(row.user_email)}, ${escapeSql(row.user_password)}, ${escapeSql(row.username)}, ${escapeSql(row.created_at)}, ${escapeSql(row.bio)}, ${escapeSql(row.profile_picture)}, ${escapeSql(row.linkedin_url)}, ${escapeSql(row.github_url)}, ${escapeSql(row.website_url)}, ${escapeSql(row.twitter_url)}, ${escapeSql(row.about)}, ${escapeSql(row.full_name)}, ${escapeSql(row.github_url)}, ${escapeSql(row.role)}) ON CONFLICT (user_id) DO UPDATE SET user_name = EXCLUDED.user_name, bio = EXCLUDED.bio, profile_picture = EXCLUDED.profile_picture, linkedin_url = EXCLUDED.linkedin_url, github_url = EXCLUDED.github_url;\n`);
        }
